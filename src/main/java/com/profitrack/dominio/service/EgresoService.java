...
public class EgresoService implements EgresoUseCase {
    ...
    public EgresoResponseDto crear(EgresoRequestDto dto) {
        ...
        CategoriaEgreso cat = dto.getCategoriaId() != null
                ? categoriaRepo.buscarPorId(dto.getCategoriaId()).orElse(null)
                : null;
    ...
    @Override
    public List<EgresoResponseDto> listarPorEmpresa(Long empresaId) {
        return egresoRepo.buscarActivosPorEmpresa(empresaId).stream().map(this::toDto).collect(Collectors.toList());
    }

    @Override
    public List<EgresoResponseDto> listarPorProyecto(Long proyectoId) {
